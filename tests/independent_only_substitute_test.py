...
NEW_RESNET_NAME = "resnet18_A_only_ablation_softgate_psnr38_badnet_ref_ratio45_50_statfixed.pth"
...
class AblationPoisonDataset(torch.utils.data.Dataset):
    def __init__(self, root, ae, train=True, mode='train_A_only'):
...
if __name__ == "__main__":
    ...
    # 3) 数据
    train_ds = AblationPoisonDataset('./data', ae, train=True, mode='train_A_only')
    ...
    print(f"\n>>> 启动 A-only 消融训练 | 设备: {DEVICE} | 注入率: {POISON_RATE*100}%")
